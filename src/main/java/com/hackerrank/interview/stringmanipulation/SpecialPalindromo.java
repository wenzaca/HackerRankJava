    ...
    private static void getResult(char[] chars, int j){
        while(j>1){
            ...
            if (chars[0]==chars[j]) {
                ...
                if(beginString.equals(endString) && beginString.replace(String.valueOf(beginString.charAt(0)), "").length() == 0 && endString.replace(String.valueOf(endString.charAt(0)), "").length() == 0) result++;
            }
        }
    }
